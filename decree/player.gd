...
func take_turn(board, target_player, stack, target_pos):
	...
	if target_pos != null and board[target_pos[0]][target_pos[1]] == null and in_short_range:
	...
	elif target_pos != null and board[target_pos[0]][target_pos[1]] and in_long_range:
		...
		if closest_dest != null and is_in_range(closest_dest, target_player.board_position, target_player.range):
			Globals.IS_PLAYER_TURN = false
			await move(board, self, closest_dest, stack)
			var push_offset = get_push_offset(self, board, board[target_pos[0]][target_pos[1]])
...
func get_push_offset(player, board, tile):
	var offset
	if board_position[0] < tile.board_position[0]:
		offset = Vector2i(1,0)
	elif board_position[0] > tile.board_position[0]:
		offset = Vector2i(-1,0)
	elif board_position[1] < tile.board_position[1]:
		offset = Vector2i(0,1)
	else:
		offset = Vector2i(0,-1)
	return offset
